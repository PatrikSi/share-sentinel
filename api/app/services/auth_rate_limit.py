...
def _failure_keys(email: str, ip: str) -> list[str]:
    ...
    return [
        f"share-sentinel:auth:fail:email:{normalized_email}",
        f"share-sentinel:auth:fail:ip:{normalized_ip}",
    ]
...
def _lock_keys(email: str, ip: str) -> list[str]:
    ...
    normalized_ip = _normalize_ip(ip)
    return [
        f"share-sentinel:auth:lock:email:{normalized_email}",
        f"share-sentinel:auth:lock:ip:{normalized_ip}",
    ]
