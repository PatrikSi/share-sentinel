...
The defaults (10,000 permission objects, 25,000 permission HTTP attempts, and 100,000 normalized entries) are the supported starting envelope, not a claim that the CLI hard ceilings are validated throughput targets. Permission requests also consume the 250,000-attempt run-wide Graph budget, so retries and pages cannot bypass the overall ceiling. The hard ceilings only reject pathological configuration. Load-test the complete collect/upload/ingest/compare path against representative data before raising defaults substantially; for very large tenants, prefer targeted-site runs with stable scope. Monitor artifact size, Graph throttling, ingest duration, PostgreSQL statement time, and permission table growth.
...
The default state location follows the platform state directory; override it with `--state-path` or `SHARE_SENTINEL_GRAPH_STATE_PATH`. Preserve this file between scheduled container runs by keeping it under the collector's `/data` volume. The collector refuses symlinks in the existing parent chain. On POSIX systems the final parent must be owned by the current user and not writable by group or others, while the database and SQLite sidecars must be current-user-owned regular files with mode `0600`. On Windows, type and symlink checks still apply, but protecting the directory and files with an ACL remains an operator responsibility. State contains filenames, paths, URLs, and IDs and should be backed up and protected accordingly. File outputs are assembled in a protected sibling spool before atomic replacement; stdout and upload-only container runs use `/data` for temporary spooling. Size that persistent volume for the state database plus both the completed artifact and its in-progress spool.
...
The synchronous run-diff API defaults to a 250,000-item combined ceiling and fails early above it rather than risking API memory exhaustion. Use the asynchronous materialized comparison workflow for larger recurring resource and item histories; its work is keyset-batched, checkpointed, and paginated, but still needs deployment-specific capacity validation. Use the repository capacity-artifact workflow to establish ingest throughput, then add a representative tenant dry run because mocked Graph tests cannot validate Microsoft-side quota and Conditional Access behavior.
