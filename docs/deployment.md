...
Synchronous item-path comparison is capped by `API_RUN_DIFF_MAX_ITEMS` (default `250000` total items across both runs). Above that envelope the API returns `422` instead of risking unbounded process memory. Large resource comparisons use the materialized worker workflow. `API_COMPARISON_MAX_ACTIVE_PER_PROJECT` defaults to `3`; comparison creation defaults to `12` starts per actor/project in a `60` second window. Resource results are batch-written, indexed for provider/category/search investigation, and keyset-paginated. Per-item added/removed/moved materialization is not implemented.
