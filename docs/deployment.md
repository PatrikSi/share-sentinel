...
SHARE_SENTINEL_IMAGE_TAG=v1.3.0
...
Synchronous item-path comparison is capped by `API_RUN_DIFF_MAX_ITEMS` (default `250000` total items across both runs). Above that envelope the API returns `422` instead of risking unbounded process memory. Large recurring comparisons use the materialized worker workflow. `API_COMPARISON_MAX_ACTIVE_PER_PROJECT` and the worker's `AUTOMATIC_COMPARISON_MAX_ACTIVE_PER_PROJECT` both default to `3` and share a serialized project admission lane; keep them aligned unless measured manual and automatic workload policy requires otherwise. Comparison creation/retry defaults to `12` starts per actor/project in a `60` second window. Resource and per-item results are batch-written, indexed for investigation, and keyset-paginated. Durable phase/cursor checkpoints and bounded work quanta let a worker recover without deleting completed batches or starving ingest indefinitely; explicit operator retry is the only path that resets a failed result.

`COMPARISON_WORK_QUANTUM_SECONDS` (default `30`, allowed `5`–`300`) and `COMPARISON_WORK_QUANTUM_BATCHES` (default `20`, allowed `1`–`1000`) bound one comparison claim before it yields to Postgres recovery. Raising either value reduces claim overhead but increases latency for other work in a serial worker; tune them only from measured comparison and ingest age.

Finding evaluation is also bounded: `FINDING_EVALUATION_BATCH_SIZE` defaults to `500` (allowed `50`–`5000`) and `FINDING_RESOLUTION_BATCH_SIZE` defaults to `250` (allowed `25`–`1000`). The former controls one policy candidate batch and the latter bounds authoritative resolution/accepted-risk maintenance. Increase either only after measuring statement time, lock pressure, and finding backlog.

Artifact part writes preserve configured free-space headroom under a cross-process POSIX advisory lock. Multi-replica storage must support `flock`, hard links, and directory `fsync`; raw known-length capacity exhaustion returns `507`, while lock/I/O unavailability returns retryable `503`. These checks do not serialize unrelated host writers, so keep reserve thresholds above measured external growth and the maximum concurrent part envelope.
...
Alembic migrations are applied forward by bootstrap. Monitoring upgrade `0014` creates source, finding, occurrence, and item-history state while staging foreign keys for existing large tables; `0015` validates those constraints and creates query indexes concurrently outside a transaction. Audit migration `0016` adds nullable retained-attribution columns and the new-row trigger; `0017` creates its lookup indexes concurrently; restart-safe `0018` installs idempotent parent triggers and backfills live-parent legacy rows in committed ID-ordered batches of at most 5,000. Back up first, pause user/token/project rename and deletion until `0018` completes, and test the full chain against production-sized data. A legacy row receives its parent's label at upgrade time, not a historical label that may already have changed; an already orphaned parent cannot be reconstructed. If bootstrap is interrupted, resolve the underlying failure and rerun it—the concurrent-index and trigger steps are recovery-safe. Automated downgrade or zero-downtime multi-version compatibility is not promised in the current release line.
...
- Run-diff detail arrays are bounded and disclose truncation; comparisons above the synchronous item envelope are rejected. The materialized path handles recurring large comparisons, but retained resource/item history grows with every completed pair and needs an explicit retention/capacity policy.
...
- The API metrics endpoint exposes durable run/comparison ages, Redis retained/pending/lag state, source/finding posture, artifact headroom, and per-component collection success. It is not a full tracing system or per-record worker-stage dashboard.
